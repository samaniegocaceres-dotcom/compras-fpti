/* ================================================================
   COMPRAS FPTI-PY — app.js
   Módulo principal: DataStore + Router + UI + Autosave + Print
...
function makeProcess(d = {}) {
  ...
  return {
    ...
    ofertas: d.ofertas || {},
    adjudicaciones: d.adjudicaciones || {},
...
function handleRoute() {
  ...
  document.getElementById('headerTitle').textContent =
    h.startsWith('#/detail/') ? 'Detalle del Proceso' : 'Índice de Procesos';
...
function renderIndex() {
  ...
  document.getElementById('printFilterInfo').textContent =
    [t && `Tipo: ${t}`, e && `Estado: ${e}`, g && `Sigla: ${g}`, s && `Búsqueda: "${s}"`].filter(Boolean).join(' | ') || 'Sin filtros';
}
...
function attachAutosaveListeners(pid) {
  ...
  document.querySelectorAll('[data-autosave-item]').forEach(el => {
    el.addEventListener('blur', () => autoSaveField(pid));
  });
  document.querySelectorAll('[data-autosave-part]').forEach(el => {
    el.addEventListener('blur', () => autoSaveField(pid));
  });
  document.querySelectorAll('[data-autosave-offer]').forEach(el => {
    el.addEventListener('blur', () => autoSaveField(pid));
  });
}
...
function removeItem(pid, idx) { const p = allProcesses.find(x => x.id === pid); if (!p) return; p.items.splice(idx, 1); const nO = {}, nA = {}; Object.entries(p.ofertas).forEach(([k, v]) => { const [pi, ii] = k.split('-').map(Number); if (ii < idx) nO[k] = v; else if (ii > idx) nO[`${pi}-${ii-1}`] = v; }); Object.entries(p.adjudicaciones || {}).forEach(([k, v]) => { const [pi, ii] = k.split('-').map(Number); if (ii < idx) nA[k] = v; else if (ii > idx) nA[`${pi}-${ii-1}`] = v; }); p.ofertas = nO; p.adjudicaciones = nA; dbPut(p).then(() => renderDetail(pid)); }
...
function removePart(pid, idx) { const p = allProcesses.find(x => x.id === pid); if (!p) return; p.participantes.splice(idx, 1); const nO = {}, nA = {}; Object.entries(p.ofertas).forEach(([k, v]) => { const [pi, ii] = k.split('-').map(Number); if (pi < idx) nO[k] = v; else if (pi > idx) nO[`${pi-1}-${ii}`] = v; }); Object.entries(p.adjudicaciones || {}).forEach(([k, v]) => { const [pi, ii] = k.split('-').map(Number); if (pi < idx) nA[k] = v; else if (pi > idx) nA[`${pi-1}-${ii}`] = v; }); p.ofertas = nO; p.adjudicaciones = nA; dbPut(p).then(() => renderDetail(pid)); }
...
async function handleImportJSON(e) {
  ...
  try {
    ...
    for (const item of data) {
      const p = makeProcess(item);
      // Preserve extra fields from import
...
// Excel import (bonus, compatible with previous version)
  ...
  try {
    ...
    if (wb.SheetNames.includes('DATA')) {
      try {
        ...
        if (j) { const parsed = JSON.parse(j); if (Array.isArray(parsed)) { await dbClear(); for (const p of parsed) await dbPut(makeProcess(p)); allProcesses = await dbAll(); toast(`Importados ${allProcesses.length} procesos (Excel completo)`); navigate('#/'); renderIndex(); e.target.value = ''; return; } }
      } catch (ex) {}
    }
    // Parse INDEX sheet
    const sd = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { header: 1, defval: '' });
    let hr = -1;
    for (let i = 0; i < Math.min(sd.length, 10); i++) { if (sd[i].map(c => String(c).toLowerCase()).some(c => c.includes('proceso') || c.includes('objeto'))) { hr = i; break; } }
    const procs = [];
    if (hr >= 0) {
      const hd = sd[hr].map(h => String(h).toLowerCase().trim());
      const cT = hd.findIndex(h => h.includes('tipo')), cP = hd.findIndex(h => h.includes('proceso')), cO = hd.findIndex(h => h.includes('objeto')), cE = hd.findIndex(h => h.includes('estado')), cEv = hd.findIndex(h => h.includes('próximo') || h.includes('proximo') || h.includes('evento')), cS = hd.findIndex(h => h.includes('sigla') || h.includes('proyecto'));
      for (let i = hr + 1; i < sd.length; i++) {
        ...
        if (raw.match(/^(CLP|CPP)/i)) { const m = raw.match(/^(CLP|CPP)[\/\s]*(.+)/i); if (m) { tipo = m[1].toUpperCase(); proc = m[2].replace(/^\//, '').trim(); } } else proc = raw;
        ...
        procs.push(makeProcess({ tipo, proceso: proc, sigla: String(r[cS >= 0 ? cS : 5] || '').trim(), objeto: String(r[cO >= 0 ? cO : 2] || '').trim(), estado: String(r[cE >= 0 ? cE : 3] || '').trim(), proximoEvento: String(r[cEv >= 0 ? cEv : 4] || '').trim() }));
      }
    }
...
function printSummary() {
  document.getElementById('printTimestamp').textContent = now();
  window.print();
}
function printDetail(pid) {
  document.getElementById('printTimestamp').textContent = now();
  document.getElementById('printFilterInfo').textContent = `Detalle: ${pid}`;
  window.print();
}
...
async function init() {
  ...
  if (!allProcesses.length) {
    try {
      const res = await fetch('./data/PROCESOS SEMANALES DE COMPRAS PTI 2026.xls');
      if (res.ok) {
